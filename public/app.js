  ...
  $('#table').innerHTML = `
    ...
    <div class="table-layout"><section class="play-area" aria-label="Poker table">
      <div class="pot-board"><div class="streets" aria-label="Betting street">${['preflop', 'flop', 'turn', 'river', 'showdown'].map(street => `<span class="${state.street === street ? 'current' : ''}" ${state.street === street ? 'aria-current="step"' : ''}>${street.toUpperCase()}</span>`).join('')}</div>
        ...
        ${state.pots.length > 1 && state.street !== 'complete' && (state.street === 'showdown' || state.players.some(p => p.inHand && !p.folded && p.stack === 0)) ? `<div class="pot-summary">${state.pots.map((p, i) => `<span>${i ? `Side pot ${i}` : 'Main pot'} · ${number(p.amount)}</span>`).join('')}</div>` : ''}</div>
    ...
    <section class="dock" aria-label="Your betting controls"><div class="dock-inner"><div class="dock-summary"><div class="turn-label ${state.legal.active ? '' : 'waiting'}" role="status">${!ready ? 'RECONNECTING' : state.legal.active ? 'YOUR TURN' : state.street === 'showdown' ? 'SHOWDOWN' : actor ? `${escape(actor.name).toUpperCase()} TO ACT` : 'MAKE YOURSELF AT HOME'}</div><div class="balance"><span>Your stack</span><strong>${number(me.stack)}</strong></div><div class="balance"><span>To call</span><strong>${number(state.legal.toCall)}</strong></div></div>
      ...
      ${raiseOpen && state.legal.canRaise ? `<form id="raise-form" class="raise-panel"><div><label for="raise-amount">${state.currentBet ? 'Raise' : 'Bet'} to · total this street</label><div class="raise-controls"><input id="raise-range" type="range" aria-label="Raise amount slider" min="${Math.min(state.legal.minRaiseTo, state.legal.maxRaiseTo)}" max="${state.legal.maxRaiseTo}" value="${Math.min(state.legal.minRaiseTo, state.legal.maxRaiseTo)}" step="1"><input id="raise-amount" aria-label="Raise to amount" type="number" min="${Math.min(state.legal.minRaiseTo, state.legal.maxRaiseTo)}" max="${state.legal.maxRaiseTo}" value="${Math.min(state.legal.minRaiseTo, state.legal.maxRaiseTo)}" step="1" required></div></div><button class="primary raise-submit" type="submit">Confirm</button><div class="quick-bets"><button type="button" data-quick="min">Min</button><button type="button" data-quick="half">½ pot</button><button type="button" data-quick="pot">Pot</button><button type="button" data-quick="all">All-in</button></div></form>` : ''}
    </div></section>`;
...
function wireTable(me, pot) {
  ...
  document.querySelectorAll('[data-quick]').forEach(el => el.addEventListener('click', () => {
    ...
    const amounts = { min, half: state.currentBet + Math.floor((pot + state.legal.fullToCall) / 2), pot: state.currentBet + pot + state.legal.fullToCall, all: max };
